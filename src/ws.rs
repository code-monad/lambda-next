...
use crate::utils::{Result, AppError, SharedState};
...
pub async fn connect_with_retry(
    ...
    loop {
        attempts += 1;
        info!("Connecting to WebSocket server at {} (attempt {})", url, attempts);
        
        match connect_to_websocket(&url).await {
            Ok(ws_stream) => {
                info!("WebSocket connection established after {} attempts", attempts);
                return Ok(ws_stream);
            },
            Err(e) => {
                if attempts > 5 {
                ...
                warn!("Connection attempt {} failed: {}. Retrying in {} seconds", 
                    attempts, e, retry_interval.as_secs());
...
pub async fn connect_to_websocket(url: &Url) -> Result<tokio_tungstenite::WebSocketStream<tokio_tungstenite::MaybeTlsStream<tokio::net::TcpStream>>> {
    match url.scheme() {
        "ws" | "wss" => {
